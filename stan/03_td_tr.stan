...
//
// Important! the m-array must be constructed after first *excluding* all
...
data {
  ...
  array[12*(T-1), 12*(T-1)+1] int<lower=0> marr;   // m-array (see above)
  // number of birds marked as adults per site and cohort that were...
...
model {
  ...
  for (i in 1:3) {
    for (t in 1:(T-1)) {
      int ind = 3 + 4*(i-1) + 12*(t-1); // Care required to get index correct
      target += N_0[i,t] * log( pr[ind][12*(T-1)+1] * pi_r[i][t] + (1 - pi_r[i][t]) );
    }
  }
}
