...
//   (iii) use a spike-and-slab prior on residency probabilities
// 
...
functions { 
  matrix get_multinomial_probs(
      ...
      for (I in 1:3) {     // I indexes source site
        ...
        for (J in 1:3) {   // J indexes destination site
          ...
          gamma[t][first_i:last_i, first_j:last_j] = [
            ...
            [ eps,                     eps,    phi_im[t]*m_jv[I][J]*p_ad_N[J][t+1],    phi_im[t]*m_jv[I][J]*(1-p_ad_N[J][t+1]) ],
    ...
    real bss_lpdf(real x) {
      ...
      real log_spike = log(w_spike) - log(w_spike + w_slab) + beta_lpdf(x|alpha_spike, beta_spike); 
      real log_slab = log(w_slab) - log(w_spike + w_slab) + beta_lpdf(x|alpha_slab, beta_slab);
      return log_sum_exp(log_spike, log_slab);
    }
}
...
parameters {
  // time- and site-varying survival probabilities for wild-raised birds and hand-reared birds  
  ...
  real hr_jv;
  real hr_ad;
  // immature survival pooled across sites and hand-rearing categories
  // with a year random effect (using non-centered parametrisation)
  real mu_logit_phi_im;
  real<lower=0> sigma_logit_phi_im;
  vector[T-1] z_logit_phi_im;
...
transformed parameters {
  ...
  vector[T-1] phi_im;
  phi_im = inv_logit(mu_logit_phi_im + sigma_logit_phi_im * z_logit_phi_im);
}

model {
  // default uniform priors on all probabilities except residency probabilities
  for (i in 1:3) {
    for (t in 1:(T-1)) {
      pi_r[i][t] ~ bss();
    }
  ...
  // priors on logit-scale immature survival hyperparameters
  mu_logit_phi_im ~ normal(0,1);
  sigma_logit_phi_im ~ exponential(2);
  z_logit_phi_im ~ normal(0,1);
  // calculate multinomial probabilities
  matrix[12*(T-1), 12*(T-1)+1] pr_wr;
  matrix[12*(T-1), 12*(T-1)+1] pr_hr;
  pr_wr = get_multinomial_probs( 
    T, eps, phi_jv_wr, phi_im, phi_ad_wr, m_jv, m_ad, p_im, p_ad_N, p_ad_A, p_ad_U
  ); 
  pr_hr = get_multinomial_probs( 
    T, eps, phi_jv_hr, phi_im, phi_ad_hr, m_jv, m_ad, p_im, p_ad_N, p_ad_A, p_ad_U
  ); 
  // likelihood
