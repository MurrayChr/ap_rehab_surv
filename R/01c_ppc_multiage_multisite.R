#' Posterior predictive checks of model fit for '01_multiage_multisite.stan' 
#' to the data 'data/00b_cmr_data_multisite_multiage.RDS'
library(tidyverse)
library(cmdstanr)
source("R/01b_fn_sim_rep_data_multiage_multisite.R")
...
real_cmr_data <- readRDS("data/00b_cmr_data_multisite_multiage.RDS")
fit <- readRDS("outputs/01a_multiage_multisite_fit.RDS")

# remove individuals first captured on last occasion (if any)
...
occasions <- 2:(T-1)
states <- c(3, 6, 9) # adult states only
...
for (r in 1:n_reps) {
  ...
  y <- rep_data$y
  y[y==10] <- 0  # turn 'not detected' code to zero
  ...
  for (occ in occasions) {
    for (st in states) {
      observed <- get_3G.SR_table(rep_cmr_data, occasion = occ, state = st, nStates = 10)
...
for (occ in occasions) {
  for (st in states) {
    observed <- get_3G.SR_table(real_cmr_data, occasion = occ, state = st, nStates = 10)
    expected <- get_expected_frequencies(observed)
...
for (r in 1:n_reps) {
  ...
  for (occ in occasions) {
    for (st in states) {
      observed <- get_WBWA_table(rep_cmr_data, occasion = occ, state = st, nStates = 9)
      expected <- get_expected_frequencies(observed)
      temp$ft_stat[(temp$occasion == occ)&(temp$state == st)] <-
        get_ft(observed, expected)
    }
...
for (occ in occasions) {
  for (st in states) {
    observed <- get_WBWA_table(real_cmr_data, occasion = occ, state = st, nStates = 10)
    expected <- get_expected_frequencies(observed)
...
for (r in 1:n_reps) {
  ...
  for (occ in occasions) {
    ...
    observed <- get_M.ITEC_table(rep_cmr_data, occ, nStates = 10)
    ...
    # containing the 'mixture' part and lower rows the 'component' part
...
for (occ in occasions) {
  # get observed tables and split into 'component' and 'mixture' parts
  observed <- get_M.ITEC_table(real_cmr_data, occ, nStates = 10)
  obs_comp <- observed[str_starts(rownames(observed), "curr"), ]
  obs_mix <- observed[str_starts(rownames(observed), "prev"), ]
