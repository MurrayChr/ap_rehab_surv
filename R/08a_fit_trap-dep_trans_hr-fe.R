# Fit model with trap-dependence, transience and independent survival probabilities
# for hand-reared and wild-raised birds (rather than an additive, logit-scale 
# hand-rearing effect).
# The purpose of fitting this model is to see how much information there is in
# the data to separately inform hand-reared and wild-raised survival in each site 
# and year (without the constant logit-scale effect that is shared across sites 
# and years).
library(tidyverse)
library(cowplot)
library(cmdstanr)
...
# m-array for model '08_td_tr_hr-fe.stan' which has 12 states for each hand-rearing category
...
fit <- readRDS("outputs/08a_td_tr_hr-fe_fit.RDS")
...
# ggsave(plot = plt_estimates, "figs/08a_estimates.pdf", height = 16.6, width = 10)
