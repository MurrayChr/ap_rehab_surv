...
#' on survival which additionally includes 
#'   (i) separate immature survival (as opposed to them sharing survival with adults)
#'   (ii) a spike-and-slab prior on residency probabilities (for details see 
#'        00_beta_spike_and_slab_engineering.R)
library(tidyverse)
library(cowplot)
library(cmdstanr)
...
# m-array for model '06_td_tr-bss_hr_phi-im.stan'  which has 12 states for each hand-rearing category
...
fit <- readRDS("outputs/06a_td_tr-bss_hr_phi-im_fit.RDS")
...
# ggsave(plot = plt_estimates, "figs/06a_estimates.pdf", height = 20, width = 10)
