...
#' '05_td_tr_hr_phi-im.stan' that incorporates trap-dependence, transience, a 
#' hand-rearing covariate on adult and juvenile survival, and immature survival
#' distinct from adult survival.
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#     ---- Function to sim data under '05_td_tr_hr_phi-im.stan' ----
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
sim_cmr_data <- function(pars, data_str) {
  ...
  pars_required <- c("phi_ad_wr", "phi_jv_wr", "hr_jv", "hr_ad", "phi_im", "pi_r", 
  ...
  if (length(pars$phi_im)!=(T-1)) {
    stop(str_c("Error! Incorrect length for 'phi_im'; should be ",T-1,"."))
  }
      ...
      for (J in 1:3) {
        ...
        trans_wr[[t]][first_i:last_i, first_j:last_j] <- matrix(
          c( 
            ...
            0,                     0,      phi_im[t]*m_jv[I,J]*p_ad_N[J,t+1],      phi_im[t]*m_jv[I,J]*(1-p_ad_N[J,t+1]),
...
# function arguments:
# 'fit' - a cmdstanr fit of model '05_td_tr_hr_phi-im.stan'
...
get_pars_from_posterior <- function( fit, draw ) {
  ...
  # 'vector_vars' confusingly has variables whose values are in site x time
  # matrices, not vectors
  vector_vars <- c("phi_jv_wr", "phi_ad_wr", "pi_r", 
                   "p_im", "p_ad_A", "p_ad_U", "p_ad_N", 
                   "m_jv", "m_ad")
  scalar_vars <- c("hr_jv", "hr_ad")
  vars <- c(scalar_vars, "phi_im", vector_vars)
  
  post_sample <- fit$draws(vars, format="df") %>%
    filter(.draw==draw) %>%
    select(-starts_with("."))
  
  # phi_im values are special case as only vary by time, not site and time
  phi_im_list <- list(
    "phi_im" = post_sample %>%
      select(starts_with("phi_im")) %>%
  ...
  pars_list <- c(vector_pars_list, phi_im_list, scalar_pars_list)
