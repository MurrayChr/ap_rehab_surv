#' Simulation test of model 04_td_tr_hr.stan
...
#'   - For wild-raised survival probabilities, movement probabilities and residency 
#'     probabilities we sample from distributions modeled on the posterior of the
#'     fitted model 04_td_tr_hr.stan
#'   - For detection probabilities we sample randomly from marginal posteriors
#'     of fitted model 04_td_tr_hr.stan
...
source("R/04b_fn_sim_rep_data_trap-dep_trans_hr.R")
...
for (i in 1:n_reps) {
  ...
  file <- str_c("outputs/sim_04/rep_data/04c_", str_pad(i, 3, "left", "0"), ".RDS")
...
get_stan_data <- function(rep_cmr_data) {
  ...
  # m-array for model '04_td_tr_hr.stan' which has 12 states
...
# Fit each replicate dataset and save to outputs/sim_04/fitted_models
mod_file <- "stan/04_td_tr_hr.stan"
mod <- cmdstan_model(mod_file)
for (i in 1:n_reps) {
  file_num <-  str_pad(i, 3, "left", "0")
  cmr_data_file <- str_c("outputs/sim_04/rep_data/04c_", file_num, ".RDS")
  rep_cmr_data <- readRDS(cmr_data_file)
  stan_data <- get_stan_data(rep_cmr_data)
  fit <- mod$sample(stan_data, parallel_chains = 4, 
                    iter_warmup = 500, iter_sampling = 500)
  fit_file <- str_c("outputs/sim_04/fitted_models/04c_", file_num, "_fit.RDS")
...
i <- 1
file_num <-  str_pad(i, 3, "left", "0")
rep_cmr_data <- readRDS(str_c("outputs/sim_04/rep_data/04c_", file_num, ".RDS"))
