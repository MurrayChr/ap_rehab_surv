...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ---- Function to sim data under '02_multiage_multisite_trap-dep_trans.stan' ----
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
sim_cmr_data <- function(pars, data_str) {
  ...
  pars_required <- c("phi_ad", "phi_jv", "pi_r", "p_im", "p_ad_A", "p_ad_U", "p_ad_N",
                     "m_jv", "m_ad")
  ...
  for (i in 1:nrow(z)) {
    if (fc_code[i] %in% c(3, 7, 11)) {  # individuals marked as adults
      ...
      # if resident then fate is sampled from transition matrices
      if (res) {
        for (t in fc[i]:(T-1)) {
...
get_pars_from_posterior <- function( fit, draw ) {
  ...
  vars <- c("phi_jv", "phi_ad", "pi_r", "p_im", "p_ad_A", "p_ad_U", "p_ad_N", 
            "m_jv", "m_ad")
